...
def traindataloader(multiscale=False, factor_scale=[8, 5], augmentation=True, path="Dataset/train", image_normalization=True,
                    box_normalization=True, input_size=(512, 512), batch_size=8, num_workers=4, shuffle=True, mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]):
    ...
    if multiscale:
        ...
        if augmentation:
            train_transform = [SSDTrainTransform_V0(x * h_seed_, x * w_seed, mean=mean, std=std) for x in range(init, end)]
        else:
            train_transform = [SSDTrainResize_V0(x * h_seed_, x * w_seed, mean=mean, std=std) for x in range(init, end)]
        ...
        if augmentation:
            train_transform = SSDTrainTransform(input_size[0], input_size[1])
        else:
