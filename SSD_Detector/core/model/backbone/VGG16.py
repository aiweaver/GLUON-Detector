    ...
    def __init__(self, layers, filters):
        ...
        with self.name_scope():
            ...
            for layer, filter in zip(layers, filters):
                stage = gluon.nn.HybridSequential(prefix='')
                with stage.name_scope():
                    for _ in range(layer):
                        stage.add(gluon.nn.Conv2D(filter,
                                                  kernel_size=3,
                                                  padding=1,
                                                  weight_initializer=mx.init.Xavier(rnd_type='gaussian',
                                                                                    factor_type='out', magnitude=3),
                                                  bias_initializer='zeros'))
                        stage.add(gluon.nn.Activation('relu'))
                self.stages.add(stage)

            # fc6, fc7 to dilated convolution layer - hybrid_forward에서 pooling 진행
            stage = gluon.nn.HybridSequential(prefix='dilated_')
            with stage.name_scope():
                # conv6(fc6) - dilated
                stage.add(gluon.nn.Conv2D(1024, kernel_size=3,
                                          padding=6,
                                          dilation=6,
                ...
                # conv7(fc7)
                stage.add(gluon.nn.Conv2D(1024, kernel_size=1,
                                          weight_initializer=mx.init.Xavier(rnd_type='gaussian', factor_type='out',
                                                                            magnitude=3),
                                          bias_initializer='zeros'))
                stage.add(gluon.nn.Activation('relu'))
...
class VGGAtrousExtractor(VGGAtrousBase):
    ...
    def __init__(self, layers, filters, extras):
        ...
        with self.name_scope():
            self.extras = gluon.nn.HybridSequential()
            for i, config in enumerate(extras):
                extra = gluon.nn.HybridSequential(prefix='extra%d_' % (i))
                with extra.name_scope():
                    for channels, kernel, strides, padding in config:
                        extra.add(gluon.nn.Conv2D(channels=channels,
                                                  kernel_size=kernel,
                                                  strides=strides,
                                                  padding=padding,
                                                  weight_initializer=mx.init.Xavier(rnd_type='gaussian',
                                                                                    factor_type='out', magnitude=3),
                                                  bias_initializer='zeros'))
                        extra.add(gluon.nn.Activation('relu'))
...
def VGG16(version=512, pretrained=False, ctx=mx.cpu(), root="modelparam"):
...
if __name__ == "__main__":
    ...
    net = VGG16(version=512, pretrained=True, ctx=mx.cpu(), root=os.path.join(root, "modelparam"))
