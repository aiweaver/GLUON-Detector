...
from core.model.backbone.VGG16 import *
...
class SSDAnchorGenerator(gluon.HybridBlock):
    ...
    def _generate_anchors(self, feature_size=None, box_size=None, box_ratio=None, box_offset=None):
        ...
        """
        # cx, cy, w, h 순으로 붙임!!!
        # box width , height scaling 하기( 0 ~ 1)

        scaled_box_first_width = box_size[0] / self._width
        scaled_box_first_height = box_size[0] / self._height

        scaled_box_second_width = box_size[1] / self._width
        scaled_box_second_height = box_size[1] / self._height
...
class ConvPredictor(gluon.HybridBlock):
    ...
    def __init__(self, num_channel=None,
        ...
        with self.name_scope():
            self.predictor = gluon.nn.Conv2D(
...
class SSD_VGG16(gluon.HybridBlock):
    ...
    def __init__(self, version=512, input_size=(512, 512),
        ...
        feature_sizes = []
        fetures_output = VGG16(version=version, ctx=mx.cpu())(
        ...
        with self.name_scope():
            ...
            self._class_predictors = gluon.nn.HybridSequential()
            self._box_predictors = gluon.nn.HybridSequential()
            self._anchor_generators = gluon.nn.HybridSequential()
...
if __name__ == "__main__":
    ...
    input size(height, width) : (300, 300) >
    class prediction shape : (8, 8096, 6) 
    box prediction shape : (8, 8096, 4) 
    anchor shape : (1, 8096, 4) 
    '''
