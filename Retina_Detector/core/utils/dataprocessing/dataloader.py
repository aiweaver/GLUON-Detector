...
from core.utils.dataprocessing.dataset import DetectionDataset, DetectionDataset_V0
...
def traindataloader(multiscale=False, factor_scale=[8,5], augmentation=True, path="Dataset/train", image_normalization=True, box_normalization=True,
                    input_size=(512, 512), batch_size=8, num_workers=4, shuffle=True, mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]):

    if multiscale:

        h_seed_ = input_size[0] // factor_scale[0]
        w_seed = input_size[1] // factor_scale[0]

        init = factor_scale[0] - (factor_scale[1] // 2)
        end = factor_scale[0] + (factor_scale[1] // 2)

        if factor_scale[1] % 2 == 1:
            end = end + 1

        dataset = DetectionDataset_V0(path=path)
        if augmentation:
            train_transform = [RetinaTrainTransform_V0(x * h_seed_, x * w_seed, mean=mean, std=std) for x in range(init, end)]
        else:
            train_transform = [RetinaTrainResize_V0(x * h_seed_, x * w_seed, mean=mean, std=std) for x in range(init, end)]

        dataloader = RandomTransformDataLoader(
            train_transform, dataset, batch_size=batch_size, interval=10, last_batch='rollover',
            shuffle=True, batchify_fn=Tuple(Stack(use_shared_mem=True), Pad(pad_val=-1), Stack()),
            num_workers=num_workers)
    else:
        if augmentation:
            train_transform = RetinaTrainTransform(input_size[0], input_size[1])
        else:
...
def testdataloader(path="Dataset/test", image_normalization=True, box_normalization=True, input_size=(512, 512),
                   batch_size=1, num_workers=4, mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]):
    dataset = DetectionDataset(path=path, input_size=input_size, mean=mean, std=std, transform=None,
                               image_normalization=image_normalization,
                               box_normalization=box_normalization)
    dataloader = DataLoader(
...
if __name__ == "__main__":
    ...
    dataloader, dataset = traindataloader(augmentation=True, path=os.path.join(root, 'Dataset', 'train'),
